...
{
    class Problem18 : IEulerProblem
    {
        #region IEulerProblem Members

        public object GetSolution()
        {
            long[][] sumGrid = new long[_triangle.Length][];
            for (int i = sumGrid.Length - 1; i >= 0 ; i--)
            {
                sumGrid[i] = new long[_triangle[i].Length];
                for (int j = 0; j < sumGrid[i].Length; j++)
                {
                    sumGrid[i][j] = _triangle[i][j];
